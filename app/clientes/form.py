...
class clientesForm(FlaskForm):
    ...
    codigo = StringField('Código', validators=[DataRequired(message='El código es obligatorio.'), length(min=4, max=4, message='El código debe tener 4 caracteres.')],  render_kw={"placeholder": "CÓDIGO DEL CLIENTE", "class": "form-control", "style": "text-transform:uppercase;"})
...
class pasajerosForm(FlaskForm):
    ...
    empresa = QuerySelectField('Empresa', query_factory=lambda: clientesModel.query.order_by(clientesModel.empresa).all(), get_label='empresa_', allow_blank=True, blank_text='Seleccione una empresa', render_kw={"placeholder": "Empresa del pasajero", "class": "form-control"})

...
class programacionModel(db.Model):
    ...
    wa_data = db.Column(JSONType, nullable=True)  # Campo para almacenar datos adicionales relacionados con WhatsApp
    # Relaciones
    ...
    def serialize(self):
        return {
            ...
            'wa_callback_button': self.wa_callback_button if self.wa_callback_button else None,
            ...
            'Ciudad_Origen': self.origen_rel.nombre,
            ...
            'operador': [{'nombre': self.operador_rel.nombres.title(), 'telefono': self.operador_rel.telefono}] if self.operador_rel else [{ 'nombre': None, 'telefono': None }],
